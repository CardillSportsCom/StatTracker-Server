				...
				.then((leagueStats) => {
					...
					for (var j = 0; j < leaguePlayers.length; j++) {
						...
						var playerAverageStats = {
							...
							FTA: ((perGame * playerStats.sum("FTA")) / playerStats.length).toFixed(2),
							FTM: ((perGame * playerStats.sum("FTM")) / playerStats.length).toFixed(2),
							fouls: ((perGame * playerStats.sum("fouls")) / playerStats.length).toFixed(2),
							gamesPlayed: playerStats.length,
							gamesWon: playerStats.filter((obj) => {
								return obj.isWin == true;
							}).length,
