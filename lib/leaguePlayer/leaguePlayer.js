...
exports.create = (request, h) => {
	return LeaguePlayer.findOne({ player: request.payload.playerId, league: request.payload.leagueId })
		.exec()
...
exports.getPlayers = (request, h) => {
	return LeaguePlayer.find({ league: request.params.leagueId }, { player: 1 })
		.populate("player")
		.populate("playerType")
		.exec()
		.then((leaguePlayers) => {
