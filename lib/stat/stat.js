...
exports.create = (request, h) => {
	return League.findById(request.payload.leagueId)
		...
		.then((league) => {
			...
			return Season.findById(request.payload.seasonId)
				.exec()
				.then((season) => {
					return Log.create(DataLog)
						.then((logdata) => {
							return TeamType.findById(request.payload.teamTypeId)
								.exec()
								.then((teamType) => {
									...
									var teamBWin = false;
									if (
										parseFloat(request.payload.teamAScore) > parseFloat(request.payload.teamBScore)
									) {
										teamAWin = true;
									} else {
										teamBWin = true;
									...
									return Team.create(TeamA)
										.then((teamAObj) => {
											return Team.create(TeamB)
												.then((teamBObj) => {
