...
function insertVideoHighlights(dateString: string, dateCreated: string): void {
	...
	VideoHighlights.insertMany(
		videoHighlights.map((video) => createVideoHighlights(video, dateString, dateCreated))
	).then(() => {
...
const dateString = "20240528/";
const dateCreated = new Date("2024-05-28T20:00:00.000Z").toISOString();
