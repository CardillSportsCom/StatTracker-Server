...
exports.get = (request, h) => {
  ...
  return BasketballStat.find({league: request.params.leagueId}).populate('game').populate('team').populate('player').exec().then((leagueGames)=>{
var GameDays = [];
      var gameDates = leagueGames.map(a=>a.game.dateCreated.toLocaleDateString()).unique();
      var games = leagueGames.map(a=>a.game).unique();
      for(var j = 0; j< gameDates.length; j++){
          ...
          var gameDateLeagueGames = leagueGames.filter(obj=>{
            var momentdate = moment(obj.dateCreated);
            var localeDate = obj.dateCreated;
              return obj.dateCreated.toLocaleDateString() == gameDates[j]
          });
