...
const mongoose = require("mongoose");
>>>>>>> 72aa380293aa33e1136db760a69c2c10d3865c42
...
exports.get = (request) => {
  ...
  return BasketballStat.find({ league: request.params.leagueId })
    ...
    .populate("player")
    .exec()
    .then(async (leagueGames) => {
      var gameDays = [];
      var seasonGames = leagueGames.filter(
        (obj) => obj.game.season == request.params.seasonId
      );
      var gameDates = seasonGames
        .map((a) => a.game.dateCreated.toLocaleDateString())
        .unique();
      ...
      for (var j = 0; j < maxlength; j++) {
        ...
        var leaguePlayers = gameDateLeagueGames
          .map((a) => a.player._id)
          .unique();
        ...
        const gameDaySummary = await GameDaySummaries.findOne({
          dateCreated: {
            $gte: start,
            $lt: end,
          },
