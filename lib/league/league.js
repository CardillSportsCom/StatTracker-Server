	...
	return LeagueType.findById(request.payload.type)
		...
		.then((type) => {
			const leagueObj = new League({
				...
				hasFouls: request.payload.hasFouls == null ? false : request.payload.hasFouls,
			});
...
exports.update = (request, h) => {
	...
	return League.findById(request.params.id)
		...
		.then((leagueObj) => {
			...
			leagueObj.hasThreePoints = request.payload.hasThreePoints;
			leagueObj.threePointValue = request.payload.threePointValue;
			leagueObj.twoPointValue = request.payload.twoPointValue;
			leagueObj.hasFouls = request.payload.hasFouls;
			leagueObj.save();
...
exports.delete = async (request, h) => {
	try {
		...
		var basketBallStats = await BasketballStat.find({ league: request.params.id });
		var games = await Game.find({ league: request.params.id });
		var teams = await Team.find({ league: request.params.id });
		var seasons = await Season.find({ league: request.params.id });
		var leaguePlayers = await LeaguePlayer.find({ league: request.params.id });

		if (basketBallStats.length > 0) {
			await BasketballStat.deleteMany({ league: request.params.id });
		}
		if (games.length > 0) {
			await Game.deleteMany({ league: request.params.id });
		}
		if (teams.length > 0) {
			await Team.deleteMany({ league: request.params.id });
		}
		if (seasons.length > 0) {
			await Season.deleteMany({ league: request.params.id });
		}
		if (leaguePlayers.length > 0) {
			await LeaguePlayer.deleteMany({ league: request.params.id });
		}

		await League.deleteOne({ _id: request.params.id });
