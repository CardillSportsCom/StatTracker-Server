...
exports.get = (request, h) => {
  ...
  return BasketballStat.find({league: request.params.leagueId}).exec().then((leagueStats) => {
    var LeagueStats = [];

      var leaguePlayers = leagueStats.map(a=>a.dateCreated.toLocaleDateString()).unique();
