...
exports.get = function (request, h) {
	...
	return BasketballStat.find({ league: request.params.leagueId })
		...
		.then(function (gameStats) {
			var playerStats = [];
			var leaguePlayers = uniqueArray(
				gameStats.map(function (a) {
					return a.player._id;
				})
			);
			for (var j = 0; j < leaguePlayers.length; j++) {
				...
				if (season) {
					var playerTotalStatsSeason = calculateTotalStats(
						playerGameStats.filter(function (obj) {
							return obj.game.season == season;
						})
					);
					var playerAverageStatsSeason = calculateAverageStats(
						playerGameStats.filter(function (obj) {
							return obj.game.season == season;
						})
					);
...
function calculateAverageStats(playerGameStats) {
	...
	return {
		...
		FTM: calculateAvgStat(dailyStats, "FTM"),
		FTA: calculateAvgStat(dailyStats, "FTA"),
		fouls: calculateAvgStat(dailyStats, "fouls"),
	};
}
function calculateTotalStats(playerGameStats) {
	return {
		FGM: sum(
			playerGameStats.map(function (obj) {
				return obj.FGM;
			})
		),
		FGA: sum(
		...
		threePointersMade: sum(
			playerGameStats.map(function (obj) {
				return obj.threePointersMade;
			})
		),
		threePointersAttempted: sum(
			playerGameStats.map(function (obj) {
				return obj.threePointersAttempted;
			})
		),
		rebounds: sum(
			playerGameStats.map(function (obj) {
